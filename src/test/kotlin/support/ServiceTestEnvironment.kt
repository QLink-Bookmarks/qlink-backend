...
private class ServicePostgreSQLContainer(imageName: String) :
...
object ServiceTestEnvironment {
  ...
  fun applicationConfig(
    flywayLocations: List<String> = listOf("db/migration"),
  ): ApplicationConfig {
    start()
  ...
  private fun isKoinUsable(): Boolean = runCatching {
    GlobalContext.get().get<DataSource>()
  }.isSuccess

  private fun testDataSourceConfig(): DataSourceConfig = DataSourceConfig(
    jdbcUrl = jdbcUrl,
    username = username,
    password = password,
  ...
  private fun testApplicationConfig(): MapApplicationConfig = MapApplicationConfig().apply {
    put("flyway.schema", SCHEMA)
...
private class OverlayApplicationConfig(
  ...
  override fun propertyOrNull(path: String) =
    overrides.propertyOrNull(path) ?: fallback.propertyOrNull(path)

  override fun config(path: String): ApplicationConfig =
    runCatching { overrides.config(path) }.getOrElse { fallback.config(path) }
  ...
  override fun toMap(): Map<String, Any> =
    (fallback.toMap() + overrides.toMap()).mapValues { requireNotNull(it.value) }
}
